if (typeof (LSE_INJECT_COMPLETED) === 'undefined') {
    var LSE_INJECT_COMPLETED = true
        /*!
         * Socket.IO v2.2.0
         * (c) 2014-2018 Guillermo Rauch
         * Released under the MIT License.
         * 
         * THIS IS THE OFFICIAL SOCKET.IO (VERSION 2.2.0)
         * FOUND HERE:
    ...
    // THE SOCKET.IO PACKAGE ENDS HERE

    if (typeof (io) !== 'function') {
        console.error('[LSE YT ANALYZER] Unable to implement socket.io...')
    } else {
        let LSE_Inject_Data = {
            channel: '',
            title: '',
        ...
        let startSocket = function () {
            if (statusVariables.Socket_Is_Running === true) {
                s.close()
                clearInterval(statusVariables.LoopVariable)
                statusVariables.LoopVariable = 'disabled'
            } else {
                statusVariables.Socket_Is_Running = true
            }

            s = io('http://' + statusVariables.data.LSE_IP + ':' + statusVariables.data.LSE_Port)

            s.on('connect', function () {
                console.log('[LSE YT Analyzer] Connected to the socket')
                statusVariables.LoopVariable = setInterval(function () {
                    if (/www\.youtube\.com\/watch/.test(window.location.href)) {
                        let player = document.querySelector('video')
                        let noChanges = true

                        // check if title has changed
                        let temp = document.querySelector("#container > h1 > yt-formatted-string").innerText
                        if (typeof (temp) === 'string' && temp !== LSE_Inject_Data.title) {
                            LSE_Inject_Data.title = temp
                            noChanges = false
                        }
                        ...
                        if (temp !== LSE_Inject_Data.currentTime) {
                            LSE_Inject_Data.currentTime = temp
                            noChanges = false
                        }

                        temp = player.duration
                        if (isFinite(temp) === false) {
                            temp = 0
                        }
                        if (temp !== LSE_Inject_Data.totalTime) {
                            LSE_Inject_Data.totalTime = temp
                            noChanges = false
                        }
                        temp = player.paused
                        if (temp !== LSE_Inject_Data.paused) {
                            LSE_Inject_Data.paused = temp
                            noChanges = false
                        }

                        // check if channel has changed
                        temp = document.querySelector("#owner-name > a").innerText
                        if (typeof (temp) === 'string' && temp !== LSE_Inject_Data.channel) {
                            LSE_Inject_Data.channel = temp
                            noChanges = false
                        }

                        // check if YouTube video is paused
                        if (player.paused === true && LSE_Inject_Data.paused === false) {
                            LSE_Inject_Data.paused = true
                            noChanges = false
                        } else if (player.paused === false && LSE_Inject_Data.paused === true) {
                            LSE_Inject_Data.paused = false
                            noChanges = false
                        }

                        temp = new URLSearchParams(window.location.search).get('v')
                        if (temp !== LSE_Inject_Data.videoId){
                            LSE_Inject_Data.videoId = temp
                            noChanges = false
                        }

                        // if changes has been done, forward the data to LSE
                        if (noChanges === false) {
                            s.emit('ScriptTalk', {
                                event: 'p-Yazaar_YouTube_Analyzer:Changes',
                                data: LSE_Inject_Data
                            })
                        }
                    }
                }, 1000)
            ...
            s.on('disconnect', function () {
                console.log('disconnected from the server (Local StreamElements)')
            })
            s.on('reconnect_attempt', () => {
                s.io.opts.transports = ['polling', 'websocket']
            })
            s.on('p-Yazaar_YouTube_Analyzer:RequestData', function () {
                s.emit('ScriptTalk', {
                    event: 'p-Yazaar_YouTube_Analyzer:Changes',
                    data: LSE_Inject_Data
                })
        ...
        chrome.storage.onChanged.addListener(function (changes, namespace) {
            ...
            if (statusVariables.data.LSE_IP !== undefined && statusVariables.data.LSE_Port !== undefined && statusVariables.data.LSE_IP !== 'undefined' && statusVariables.data.LSE_Port !== 'undefined') {
                ...
                s.close()
                if (statusVariables.LoopVariable !== 'disabled') {
                    clearInterval(statusVariables.LoopVariable)
                    statusVariables.LoopVariable = 'disabled'
                }
            }
